...
def downloadImg(imgeurl, name=None, signpath=''):
    ...
    # img = GetHtml(imgeurl)
    if img is not None:
        paths = os.getcwd() + os.sep + "attachment" + os.sep + signpath + os.sep
        # 判断路径是否存在
...
def ProcessWork(furl):
    childurl = fatherurl_Get_childurl(furl)
    childurllen = len(childurl)
    for i in range(childurllen):
        p = multiprocessing.Process(target=put_childurl, args=(childurl[i],))
        p.start()
        p.join()
...
if __name__ == "__main__":
    ...
    # fatherurl.reverse()
    # 随机排序
    random.shuffle(fatherurl)
    # 创建进程池
    p = multiprocessing.Pool(5)
    for furl in fatherurl:
        p.apply_async(thredWork, args=(furl,))
    p.close()
    p.join()
